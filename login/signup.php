            ...
            <article>
                ...
                <section>
                    <div>
						...
						?>
						<h1>Sorry</h1>
						<p>You didn't fill out the form completely. <a href="/chef/login/signup.html">Try again?</a></p>

						<?php 	// 비밀번호와 비밀번호 확인란에 입력한 값 같은지.
						} else if ($pw != $pwcheck){	
						?>
						<h1>Sorry</h1>
						<p>PW and PW Check aren't match. <a href="/chef/login/signup.html">Try again?</a></p>

						<?php 	// email 형식이 맞는지 확인.
						} else if (!preg_match("/^[A-Za-z0-9_%+-]+@[A-Za-z]+\.[A-Za-z]{2,}$/",$e_mail)) { 
						?>
						<h1>Sorry</h1>
						<p>Write Correct E-mail Address. <a href="/chef/login/signup.html">Try again?</a></p>

						<?php 	// 아이디가 중복되었을 때
						} else if ($check) {
						?>
						<h1>Sorry</h1>
						<p>Your ID exists. <a href="/chef/login/signup.html">Try again?</a></p>
